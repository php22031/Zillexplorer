...
if ( $_POST['submit_reset'] ) {
	...
	mysqli_free_result($result);
	   	...
	   	if ( !$reset_key ) {
			$reset_result['error'][] = "No account exists with the email address '".$_POST['email']."'.";
			}
...
Please confirm your recent account password reset request for the email address ".$_POST['email'].". To reset your account password, please visit this link below:
...
";
	...
	$mail_result = safe_mail( $_POST['email'], "Please Confirm To Reset Your Account", $message);
...
?>
				...
				<p><b>Email:</b> <input type='text' id='email' name='email' value='<?=$_POST['email']?>' /></p>
